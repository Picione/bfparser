          ...
          if (objectSP) {
					...
					for (pk=0;pk<bbAddArray.length;pk++) {
						if (objectSP["effects"][pj][bbAddArray[pk]]) {
	                      if (lsSkillFound != 0)
    	                    functionSTR+=skillConnect;
        	              lsSkillFound+=1;
  						  functionSTR+="Add to "+bbArray[pk].toUpperCase()+" ["+findTG(objectSP["effects"][pj][bbAddArray[pk]])+"]";
						}
					}
            ...
            if (lsSkillFound < objectSP["effects"].length) {
              ...
              for (n=0;n<objectSP["effects"].length;n++) {
              	if (objectSP["effects"][n]["unknown passive id"]) {
              	functionSTR+='(passiveid:'+objectSP["effects"][n]["unknown passive id"]+';param:'+objectSP["effects"][n]["unknown passive params"]+')';
              	}
              	if (objectSP["effects"][n]["unknown proc id"]) {
              	functionSTR+='(procid:'+objectSP["effects"][n]["unknown proc id"]+';param:'+objectSP["effects"][n]["unknown proc params"]+')';
              	}
              }
              functionSTR+=']';
            }
